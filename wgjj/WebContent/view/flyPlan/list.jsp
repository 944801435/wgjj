	   	...
	   	<template>
			...
			<div class="right_content_all">
				...
				<div class="right_content_btnbox">
					...
					<div v-if="optFlag" @click="add()" class="right_content_btnbox_btn right_content_btnbox_add">
			         ...
			         <table class="table table-bordered table_list table-striped">
						...
						<tbody>
							<tr v-for="item in list">
								...
								<td>
                                    <a v-if="item.planSts==BIZ_SPACE_REQ_AUDIT" href="javascript:void(0);" @click="reCall(item.planSeq)">撤回</a>
                                    <a v-if="item.planSts==BIZ_SPACE_REQ_CREATE" href="javascript:void(0);" @click="commit(item.planSeq)">上报</a>
                                    <a v-if="item.planSts==BIZ_SPACE_REQ_CREATE" href="javascript:void(0);" @click="edit(item.planSeq)">编辑</a>
									<a href="javascript:void(0);" @click="detail(item.planSeq)">查看</a>
									<a href="javascript:void(0);" @click="editCode(item.note)"><br/>填写二次雷达码</a>
									<a href="javascript:void(0);" @click="editAcid(item.note)"><br/>填写航班号</a>
									<a v-if="item.rptPdfFileId!=null" href="javascript:void(0);" @click="downloadRpt(item.rptPdfFileId)"><br/>下载许可</a>
                                    <a href="javascript:void(0);" @click="download(item.planSeq)"><br/>导出审批结果包</a>
								</td>
							</tr>
						</tbody>
					</table>
			...
			<form id="acidForm" hidden="hidden">
				<div>
					<div class="right_content_all">
						<div class="right_content_table">
							<table class="open_table">
								<tr>
									<td>
										<div class="control-group">
											...
											<div class="controls">
												<input id="acid" name="acid" v-model="note.acid" autocomplete="off" type="text" dataType="Require,Limit" len="50" msg="请输入(1~50)个字符的航班号！" maxlength="50" class="required" value=""/>
											</div>
										</div>
									</td>
								</tr>
  ...
  $(function(){
	...
	var vm=new Vue({
		...
		data:{
            ...
            BIZ_SPACE_REQ_CREATE: "${fns:findKey('BIZ_SPACE_REQ_CREATE')}",
		...
		methods:{
			...
			//填写航班号
			editAcid(item){
				Validator.Validate_onLoad($('#acidForm')[0],3);
				vm.note = Object.assign({}, item)
				layer.open({
					type: 1
					,title: '填写航班号'
					...
					,btn: ['确定','取消']
					,yes: function(index,layero){
						if(!Validator.Validate($('#acidForm')[0],2)){
							return;
						}
						openLoading();
						$.ajax({
							url:ctx+'/flyPlan/updateAcid.action',
							data: {
								noteSeq: vm.note.noteSeq,
								acid: vm.note.acid
							},
