...
public class MD5 {
	...
	public static String getMD5(String inbuf) {
		try {
			MessageDigest md = MessageDigest.getInstance(STR_MD5);
			return bytesToHexString(md.digest(inbuf.getBytes(CHAR_SET))).toUpperCase();
		} catch (Exception e) {
			log.error("生成消息签名信息失败！", e);
			return "";
		}
		// if(md5==null)
		// {
	...
	private static String bytesToHexString(byte[] bytes) {
		...
		for (int i = 0; i < bytes.length; i++) {
			sTemp = Integer.toHexString((int) bytes[i] & 0xff);
			if (sTemp.length() < 2)
				sb.append(0);
			sb.append(sTemp);
		}
	...
	// System.out.println(args[0]+"|"+getMD5(args[0]));
	// }
	public String getMD5Imp(String inbuf) {
		md5Init();
		md5Update(inbuf.getBytes(), inbuf.length());
		md5Final();
		digestHexStr = "";
		StringBuffer digestHexStrBuf = new StringBuffer();
		for (int i = 0; i < 16; i++) {
			digestHexStrBuf.append(byteHEX(digest[i]));
		}
		digestHexStr = digestHexStrBuf.toString();
		return digestHexStr;
	}
	...
	public static String getPrintPwd(String str) {
		String md5 = MD5.getMD5(str);
		// 取3、4、7、16、19、25、27、30位
		StringBuffer buf = new StringBuffer();
		buf.append(transHexToDigital(md5.charAt(2)));
		buf.append(transHexToDigital(md5.charAt(3)));
		buf.append(transHexToDigital(md5.charAt(6)));
		buf.append(transHexToDigital(md5.charAt(15)));
		buf.append(transHexToDigital(md5.charAt(18)));
		buf.append(transHexToDigital(md5.charAt(24)));
		buf.append(transHexToDigital(md5.charAt(26)));
		buf.append(transHexToDigital(md5.charAt(29)));
		return buf.toString();
	}

	private static char transHexToDigital(char c) {
		if (c == 'A')
			c = '1';
		else if (c == 'B')
			c = '2';
	...
	public static void main(String[] args) {
//		String s="timestamp=1544689920498&sn=P76DCL08B42539&cmd=01&MsgKey=192006250b4c09247ec02edce69f6a2d";
//		System.out.println(MD5.getMD5(s).toUpperCase());
//		System.out.println(new java.util.Date().getTime());
//		
//		System.out.println("*****");
		String userName="mhadmin";
		String userPwd="111111";
		System.out.println(MD5.getMD5(userName + userPwd));
	}
}
